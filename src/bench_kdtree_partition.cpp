...
static void test_approx(float *massQ, size_t vecsize, size_t qsize, IndexKDTreePartition &appr_alg,
                        size_t vecdim,
...
static void test_vs_recall(float *massQ, size_t vecsize, size_t qsize, IndexKDTreePartition &appr_alg,
                           size_t vecdim,
                           vector<std::priority_queue<std::pair<float, labeltype >>> &answers, size_t k,
                           vector<BoundingBox> &filters) {
    vector<size_t> efs;
...
int main(int argc, char *argv[]) {
    ...
    while (iarg != -1) {
        iarg = getopt_long(argc, argv, "d:s:f:m:", longopts, &ind);
        switch (iarg) {
            case 'd':
    ...
    cout << "  Nodes: " << index.get_num_nodes()
         ...
         << ", scan: " << index.get_num_scan_nodes() << ")" << endl;
