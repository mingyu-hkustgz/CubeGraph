...
int main(int argc, char* argv[]) {
    if (argc < 5) {
        ...
        cerr << "Example: " << argv[0] << " data.fvecs metadata.bin index.grid 2" << endl;
        return 1;
    }
    ...
    cout << "=== Hierarchical Grid Index Test ===" << endl;
    ...
    // Build index
    cout << "\nBuilding hierarchical grid index..." << endl;
    auto start = chrono::high_resolution_clock::now();

    IndexGridND index(attr_dim, vec_dim);
    ...
    auto results = loaded_index.range_search(query_vec.data(), query_bbox, k, 100);
