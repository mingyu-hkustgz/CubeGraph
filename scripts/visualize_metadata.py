...
def plot_2d(data, output_path, title):
    ...
    ax.scatter(data[:, 0], data[:, 1], alpha=0.1, s=1, c='blue')
...
def plot_3d(data, output_path, title):
    ...
    for i, (elev, az) in enumerate(angles):
        ...
        ax.scatter(sample[:, 0], sample[:, 1], sample[:, 2], alpha=0.2, s=1, c='blue')
        ax.set_xlabel('Dim 0')
        ax.set_ylabel('Dim 1')
        ax.set_zlabel('Dim 2')
...
def plot_4d_pca(data, output_path, title):
    ...
    fig, ax = plt.subplots(1, 1, figsize=(8, 8))
    scatter = ax.scatter(projected[:, 0], projected[:, 1], alpha=0.2, s=1, c='blue')
    ax.set_xlabel(f'PC1 ({pca.explained_variance_ratio_[0]*100:.1f}%)')
    ax.set_ylabel(f'PC2 ({pca.explained_variance_ratio_[1]*100:.1f}%)')
    ax.set_title(f'{title}\n(PCA projection, total var explained: {sum(pca.explained_variance_ratio_[:2])*100:.1f}%)')
    plt.tight_layout()
    plt.savefig(output_path, dpi=150)
    plt.close()
...
def plot_4d_pairwise(data, output_path, title):
    ...
    for ax, (i, j) in zip(axes, pairs):
        ax.scatter(sample[:, i], sample[:, j], alpha=0.1, s=1, c='blue')
...
def plot_distribution_comparison(output_dir):
    ...
    for dim in dims:
        ...
        for idx, dist in enumerate(distributions):
            ...
            if dim == 2:
                axes[idx].scatter(sample[:, 0], sample[:, 1], alpha=0.2, s=1, c='blue')
                axes[idx].set_xlim(0, 1)
                axes[idx].set_ylim(0, 1)
                axes[idx].set_aspect('equal')
            else:
                # For 3D/4D, just show histogram of first dimension
                axes[idx].hist(sample[:, 0], bins=50, alpha=0.7, color='blue')
                axes[idx].set_xlim(0, 1)

            axes[idx].set_title(f'{dist.capitalize()} ({dim}D)')
            axes[idx].set_xlabel('Value')

        plt.suptitle(f'Metadata Distributions - {dim}D', fontsize=14)
        plt.tight_layout()
